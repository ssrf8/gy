...
const order = ['[mvu_update] 变量更新规则', '[mvu_update] 变量输出格式', '[mvu_update] 初始变量',
  ...
  '[mvu_plot] 家族·王家', '[mvu_plot] 家族·李家', '[mvu_plot] 家族·张家', '[mvu_plot] 家族·陈家'];
const defs = {
  ...
  '[mvu_plot] 家族·王家': { constant: false, enabled: true, selective: true, keys: ['王家'], comment: '[mvu_plot] 家族·王家' },
  '[mvu_plot] 家族·李家': { constant: false, enabled: true, selective: true, keys: ['李家'], comment: '[mvu_plot] 家族·李家' },
  '[mvu_plot] 家族·张家': { constant: false, enabled: true, selective: true, keys: ['张家'], comment: '[mvu_plot] 家族·张家' },
  '[mvu_plot] 家族·陈家': { constant: false, enabled: true, selective: true, keys: ['陈家'], comment: '[mvu_plot] 家族·陈家' },
};
const contentOf = {
...
// ---- 正则（prompt-only：开局页 UI 不进模型上下文）----
const regexScripts = [{
  id: '1102f8b1-0000-4000-8000-000000000001',
  scriptName: '开局页送模替换（prompt-only）',
  findRegex: '<!--gg-opening-begin-->[\\s\\S]*?<!--gg-opening-end-->',
...
const card = {
  ...
  data: {
    name: '关公',
    description: '{{char}}是{{user}}祖传玉佩中的守护灵。开局时{{user}}可从两位守护灵中选择其一——关羽（武圣之力：刀法通神、威压邪祟）或赵公明（财神之力：招财进宝、镇宅安民）；选定后{{char}}以所选守护灵的身份显现、对话与显灵。四家（王家、李家、张家、陈家）皆虔诚信奉玉佩守护灵，无论{{user}}如何切换，四家信奉始终跟随当前守护灵。中元节夜，百鬼巡街。',
    personality: '取决于所选守护灵：关羽重义刚直、言出必行；赵公明圆融通达、爱财有道。详见世界书守护灵条目。',
    scenario: '中元节夜，{{user}}的祖传玉佩忽然觉醒，两道守护灵虚影浮现，等待{{user}}选择其一。选定后，故事从四家之一展开，{{user}}自由输入开始。',
    first_mes: firstMes,
    mes_example: '',
